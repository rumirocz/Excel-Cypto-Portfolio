...
Public Const timeToUpdate As String = "00:01:30"
Private schedule As Date
...
'Time function to refresh your data (Starts timer)
Sub startTimeFunction()
    On Error Resume Next
    
    schedule = Now + TimeValue(timeToUpdate)
    
    Application.OnTime EarliestTime:=schedule, Procedure:="sheetsRecalculate", schedule:=True
End Sub

'Time function to refresh your data (Stops timer)
Sub stopTimeFunction()
    On Error Resume Next
    
    Application.OnTime EarliestTime:=schedule, Procedure:="sheetsRecalculate", schedule:=False
End Sub

'Recalculate function for time functions
Sub sheetsRecalculate()
    On Error Resume Next
